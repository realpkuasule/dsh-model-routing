...
1. **会话第一轮自动启动**：启用本技能的任何会话里，收到第一条用户消息就立即加载并运行本 skill，不需要用户说"帮我选路"、/route 等专门话术。
2. **非任务消息保持安静**：第一条消息若是寒暄、闲聊、反问、澄清请求（没有可判定"任务特质"的内容），跳过选路报告，正常继续对话，不要硬输出报告。
3. **任务消息直接判定**：第一条消息带任务时，按「选路流程」执行并输出报告。
4. **中途显式触发**：会话中途用户问"该用哪个模型/模式"或说"帮我选路"、/route、/model-route，同样执行选路流程。
5. 任务信息不足无法判定时，先问**一个**澄清问题（任务规模/复杂度/目标），不要连环提问。
